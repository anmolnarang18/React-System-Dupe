...
export default function TaskCreation({ navigation, route }) {
  ...
  const getTasks = async () => {
    try {
      let result = await apiClient.get("/task/getTasks");
      ...
      console.log("ALL TASKS FETCHING ERROR", error);
    }
  };
  ...
  const handleTaskCreation = async () => {
    ...
    try {
      const result = await apiClient.post("/task/addTask", taskDetails);

      if (result) {
        route.params.handleTaskFetching(TASK_STATUS.NOT_STARTED);
        navigation.goBack();
      }
    } catch (error) {
